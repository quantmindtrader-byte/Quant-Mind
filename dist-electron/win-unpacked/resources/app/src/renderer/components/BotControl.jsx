  ...
  useEffect(() => {
    ...
    const loadConfigs = async () => {
      try {
        ...
        if (result.success && result.data?.mt5_configs) {
          const configs = Object.keys(result.data.mt5_configs);
        ...
        console.error('Failed to load MT5 configs:', error);
      }
    };
  ...
  const handleStartBot = async () => {
    ...
    try {
      ...
      try {
        ...
        await fetch('http://74.162.152.95/api/user/bot-actions', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            action: 'start',
            device_info: `Desktop App - ${navigator.platform} - ${navigator.userAgent}`
          })
        });
      } catch (logError) {
        console.error('Failed to log bot start:', logError);
      }
  ...
  const handleStopBot = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to log bot stop:', logError);
      }
