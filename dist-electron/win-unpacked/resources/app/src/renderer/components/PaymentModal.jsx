...
const PaymentModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const fetchPlans = async () => {
    try {
      const response = await fetch('http://74.162.152.95/api/payment/plans');
  ...
  const fetchDemoCredentials = async () => {
    try {
      ...
      const response = await fetch('http://74.162.152.95/api/payment/demo-credentials', {
  ...
  const initiatePayment = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch('http://74.162.152.95/api/payment/initiate', {
        method: 'POST',
        headers: {
  ...
  const simulatePaymentSuccess = async (orderId) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch('http://74.162.152.95/api/payment/verify', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
